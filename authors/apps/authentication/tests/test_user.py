...
class TestUser(BaseTest):
    ...
    def test_logging_in(self):
        ...
        self.assertIn(self.login_data['user']['email'],
        response.data['email'])
        assert "token" in response.data

    def test_registration_without_username(self):
        """ Test signup with without a username """
    ...
    def test_registration_with_short_username(self):
        """ Test signup with a username less than 5 characters """

        response = self.client.post(
            "/api/users/", self.short_username, format="json")
    ...
    def test_registration_with_invalid_email(self):
        ...
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(
            'A valid email address is required to signup', 
            response.json()['errors']['email'])
    ...
    def test_confirm_reset(self):
        ...
        x = self.client.get(
            "/api/password-reset/{}/".format(token))
        token = x.data['token']
        self.client.credentials(HTTP_TOKEN=token)
        response = self.client.put(
            "/api/password/reset/done/", data=self.password_update, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
